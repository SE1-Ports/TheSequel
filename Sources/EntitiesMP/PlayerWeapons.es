...
290 BOOL m_bSecFireWeapon = FALSE,    // secondary weapon is firing
291 BOOL m_bPrimaryFire = FALSE,      // was this primary or secondary fire
...
//    plan.m_fRecoilSpeed += wpn_fRecoilSpeed[m_iCurrentWeapon];
  ...
  BOOL HoldingSecondaryFire()
  {
    return m_bSecFireWeapon && !m_bChangeWeapon;
  }
  ...
  void UpdateTargetingInfo(void)
  {
    ...
    if( m_penRayHit!=NULL)
    {
      ...
      if( pen->GetFlags()&ENF_ALIVE)
      {
        ...
        if( m_bFireWeapon || m_bSecFireWeapon) { m_tmTargetingStarted = 0; }
      }
      // if not alive
      else
      {
...
//      m_iNukeBalls += MAX_NUKEBALLS*GetSP()->sp_fAmmoQuantity;
        m_iPlasma += MAX_PLASMA*GetSP()->sp_fAmmoQuantity;
        m_iDev += MAX_DEV*GetSP()->sp_fAmmoQuantity;
  ...
  Idle() {
    ...
    wait() {
      on (EBegin) : {
        ...
        // secondary fire pressed start firing
        if(m_bSecFireWeapon) {
          jump SecondaryFire();
        }
        // reload pressed
